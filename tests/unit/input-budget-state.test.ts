...
describe("input, budget and state", () => {
  it("splits multiple pins on blank lines", () => {
    expect(parsePins("pin one\n\npin two\ncontinued")).toEqual(["pin one", "pin two\ncontinued"]);
  });

  it("rejects missing and excessive pins", () => {
    expect(() => parsePins("   ")).toThrow();
    expect(() => parsePins(Array.from({ length: 11 }, (_, i) => `pin ${i}`).join("\n\n"))).toThrow();
  });
  ...
  it("calculates model cost from actual usage buckets", () => {
    expect(estimateCost({ inputTokens: 1_000_000, cachedInputTokens: 1_000_000, outputTokens: 1_000_000 })).toBe(2.275);
  });
