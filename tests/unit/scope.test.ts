...
describe('scope policy', () => {
  it('normalizes public URLs and removes tracking parameters', () => {
    expect(
      normalizePublicUrl('HTTPS://Example.COM/post?utm_source=x&id=1#part'),
  ...
  it('requires the exact configured guild, channel and owner', () => {
    ...
    expect(
      isAuthorized({ guildId: 'g', channelId: 'c', userId: 'u' }, expected),
    ).toBe(true);
    expect(
      isAuthorized({ guildId: 'g', channelId: 'c', userId: 'other' }, expected),
    ).toBe(false);
  });
});
