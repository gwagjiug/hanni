...
export async function handleModal(
  ...
  const previous = JSON.parse(row.draft_json) as ArchiveDraft;
  ...
  const categoryMode = extractCategories(previous.readmeBefore).includes(candidate.data.category)
    ? "existing"
    : "new";
  const updated: ArchiveDraft = {
    ...
    pinPath,
    pinContent: renderPinFile(candidate.data.title, previous.url, candidate.data.pins),
