...
import { errorResponse, InteractionResponseType, jsonResponse } from "../../tools/discord";
...
export async function handleCommand(
  ...
  try {
    ...
    const pinNames = ["pin", ...Array.from({ length: 9 }, (_, index) => `pin-${index + 2}`)];
    pins = validatePins(pinNames.flatMap((name) => {
      const value = options.find((option) => option.name === name)?.value ?? "";
      return value ? parsePins(value) : [];
    }));
  } catch (error) {
    return errorResponse(error instanceof Error ? error.message : "Pin 입력이 올바르지 않아요.");
  }
  const runId = crypto.randomUUID();
  const defer = (promise: Promise<unknown>) => ctx.waitUntil(promise);
  ctx.waitUntil(createAndAnalyze(env, interaction, runId, url, pins, "", defer));
  return jsonResponse({
