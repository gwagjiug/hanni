...
export function parsePins(input: string): string[] {
  ...
  if (!raw) throw new Error("Pin을 한 개 이상 입력해주세요.");
...
export function validatePins(pins: string[]): string[] {
  ...
  if (pins.reduce((total, pin) => total + pin.length, 0) > MAX_TOTAL_PIN_CHARS)
    throw new Error(`Pin 전체 길이는 ${MAX_TOTAL_PIN_CHARS}자 이하여야 합니다.`);
  return pins;
}
