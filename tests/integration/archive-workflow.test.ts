...
function dependencies(initial: RunRow) {
  ...
  const store: ArchiveWorkflowDependencies["store"] = {
    ...
    complete: vi.fn(async (_id, branch, prUrl) => {
      current = { ...current, status: "COMPLETED", github_branch: branch, github_pr_url: prUrl };
    }),
  };
  const deps: ArchiveWorkflowDependencies = {
    ...
    github: {
      ...
      createDraftPr: vi.fn(async () => ({ branch: "hanni/branch", prUrl: "https://github.test/pr/1" })),
    },
    discord: {
      editOriginal: vi.fn(async () => ({ id: "message", channel_id: "channel" })),
      createThread: vi.fn(async () => ({ id: "thread" })),
      sendMessage: vi.fn(async () => undefined),
    },
    metadata: vi.fn(async () => ({ title: "Useful Agent", canonicalUrl: "https://example.com/article" })),
    prepare: vi.fn(async () => ({
      preparation: {
...
describe("archive workflow", () => {
  it("analyzes once and stores an approval-ready deterministic draft", async () => {
    ...
    await analyzeArchiveUrl(env, { runId: "run-12345678", pins: ["원문 그대로"] }, setup.deps);
  ...
  it("creates a PR only after approval and completes the run", async () => {
    ...
    const setup = dependencies(row({ status: "AWAITING_APPROVAL", draft_json: JSON.stringify(draft) }));
  ...
  it("stops before Discord and GitHub writes when token usage exceeds the circuit", async () => {
    ...
    await analyzeArchiveUrl(env, { runId: "run-12345678", pins: ["pin"] }, setup.deps);
