...
describe("web metadata", () => {
  it("extracts title and a safe canonical URL", async () => {
    ...
    await expect(fetchMetadata("https://example.com/source", fetcher)).resolves.toEqual({
      title: "Example & Test",
  ...
  it("blocks redirects to private addresses", async () => {
    ...
    await expect(fetchMetadata("https://example.com", fetcher)).rejects.toThrow("blocked_redirect_target");
  });
});
