...
describe("OpenAI boundary", () => {
  it("accepts only the narrow structured result and reports usage", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({
      output_text: JSON.stringify({
        category: { name: "AI & Engineering", mode: "existing", rationale: "engineering" },
    ...
    expect(result.usage).toEqual({ inputTokens: 100, cachedInputTokens: 10, outputTokens: 20 });
  ...
  it("retries malformed structured output once and stops", async () => {
    const fetcher = vi.fn<typeof fetch>().mockImplementation(async () =>
      new Response(JSON.stringify({ output_text: "not-json", usage: {} }), { status: 200 }),
    );
    await expect(prepareArchiveEntry({
      apiKey: "x", model: "m", title: "t", hostname: "example.com", categories: [], pins: ["p"], fetcher,
