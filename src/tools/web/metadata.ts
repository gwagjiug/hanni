  ...
  for (let redirect = 0; redirect <= 3; redirect++) {
    ...
    try {
      response = await fetcher(current, {
        ...
        headers: {
          'user-agent': 'Hanni/0.1 (+https://github.com/gwagjiug/hanni)',
          accept: 'text/html',
        },
      });
    } finally {
    ...
    if (!contentType.includes('text/html')) {
      return { title: null, canonicalUrl: current };
    }
    ...
    if (canonicalMatch?.[1]) {
      ...
      if (isAllowedHostname(candidate.hostname)) {
        canonicalUrl = normalizePublicUrl(candidate.toString());
      }
    }
    return {
