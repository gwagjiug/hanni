    ...
    try {
      ...
      this.defer(
        this.export({
          ...
          ended: Date.now(),
          status,
          attributes: redact({ ...attributes, 'hanni.run_id': this.runId }),
        }),
      );
    }
  }

  private async export(item: ExportSpan): Promise<void> {
    if (!this.env.OTEL_EXPORTER_OTLP_ENDPOINT) {
      return;
    }
    const headers: Record<string, string> = {
      'content-type': 'application/json',
    };
    if (this.env.OTEL_EXPORTER_OTLP_HEADERS) {
      headers.authorization = this.env.OTEL_EXPORTER_OTLP_HEADERS;
    }
    const traceId = this.runId.replace(/-/g, '').padEnd(32, '0').slice(0, 32);
    const spanId = crypto.randomUUID().replace(/-/g, '').slice(0, 16);
    const attributes = Object.entries(item.attributes).flatMap(
      ([key, value]) => {
        if (value === undefined) {
          return [];
        }
      ...
      resourceSpans: [
        {
          resource: {
            attributes: [
              { key: 'service.name', value: { stringValue: 'hanni' } },
            ],
          },
          scopeSpans: [
            {
    ...
    await this.fetcher(this.env.OTEL_EXPORTER_OTLP_ENDPOINT, {
      method: 'POST',
      headers,
