...
describe("Netlify AX evaluation", () => {
  ...
  it("treats answering an unrelated message as agent failure", () => {
    expect(scopeAdherence({ outOfScope: true, responseGenerated: false, llmCalls: 0, toolCalls: 0, runCreated: false })).toBe(true);
    expect(scopeAdherence({ outOfScope: true, responseGenerated: true, llmCalls: 1, toolCalls: 0, runCreated: true })).toBe(false);
  });
});
